...
import {Brain, ChevronDown, ChevronRight, Loader2, Sparkles, Wrench,} from "lucide-react";
...
function MessageRow({ message, session }) {
    if (message.role === "user")
        return <UserMessage message={message} session={session} />;
 ...
 */
const AssistantMessage = observer(function AssistantMessage({
    message,
    session,
}) {
    ...
    const hasContent = parts.some(
        (p) => (p.type === "text" && p.text) || p.type === "tool-call"
    );
        ...
        <div className="anim-rise mb-5 flex gap-3">
            ...
            <div className="min-w-0 flex-1">
                ...
                {thinking && (
                    <ThinkingBlock
                        text={thinking}
                        live={onlyThinking || (!hasContent && streaming)}
                    />
                )}
